...
class MCAIV(BaseEstimator,TransformerMixin):
    """
    ...
    -----
    ```python
    >>> MCAIV(iv = None, excl=None, n_components = 5, ind_weights = None, parallelize=False)
    ```
    ...
    * Lebreton, J. D., Sabatier, R., Banco G. and Bacou A. M. (1991) Principal component and correspondence analyses with respect to instrumental variables : an overview of their role in studies of structure-activity and species- environment relationships. In J. Devillers and W. Karcher, editors. Applied Multivariate Analysis in SAR and Environmental Studies, Kluwer Academic Publishers, 85--114.
    ...
    See Also
    --------
    `get_mcaiv_ind`, `get_mcaiv_var`, `get_mcaiv`, `summaryMCAIV`, `fviz_mcaiv_ind`, `fviz_mcaiv_var`, `fviz_mcaiv_biplot`

    Examples
    --------
    ```python
    >>> from scientisttools.datasets import poison
    >>> from scientisttools import MCAIV, summaryMCAIV
    ...
    >>> #specific multiple correspondence analysis with instrumental variables
    >>> res_spemcaiv = MCAIV(iv=(0,1),excl=(0,2))
    >>> res_spemcaiv.fit(poison)
    >>> summaryMCAIV(res_spemcaiv)
    ```
    """
    def __init__(self,
                 iv = None,
    ...
    def fit(self,X:DataFrame,y=None):
        ...
        #---------------------------------------------------------------------------------------------------------------------------------------------------------------------
        all_cat = all(api.types.is_string_dtype(y[k]) for k in y.columns)
        if not all_cat:
        ...
        ind_weights =  Series(ind_weights,index=y.index,name="weight")
        ...
        xs = x.copy()
        for k in xs.columns:
            if api.types.is_numeric_dtype(xs[k]):
                #compute weighted average and standard deviation
                d = DescrStatsW(xs[k],weights=ind_weights,ddof=0)
                #standardization : Z = (X - mu)/sigma
                xs[k] = mapply(xs[k].to_frame(),lambda x : (x - d.mean)/d.std,axis=1,progressbar=False,n_workers=n_workers)

        #multiple correspondence analysis (MCA)
        res_mca0 = MCA(excl=self.excl).fit(y)
        ncomp = int((res_mca0.eig_.iloc[:,0].values > 1e-10).sum())
        res_mca1 = MCA(excl=self.excl,n_components=ncomp).fit(y)
        ...
        def olsiv(i):
            def x_cast(k):
                if api.types.is_numeric_dtype(xs[k]):
                    return xs[k]
                if api.types.is_string_dtype(xs[k]):
                    return get_dummies(xs[k],drop_first=True,dtype=int)
            features = concat(map(lambda k : x_cast(k=k),xs.columns),axis=1)
            ols = sm.WLS(endog=coord[i].astype(float),exog=sm.add_constant(features),weights=ind_weights).fit()
            return Series(ols.fittedvalues,index=coord.index,name=i)
        yhat = concat(map(lambda i : olsiv(i=i),coord.columns),axis=1) 

        #apply Principal Component Analysis (PCA)
        sup_var_label = [*y.columns.tolist(),*x.columns.tolist()]
        res = PCA(standardize=False,n_components=self.n_components,ind_weights=ind_weights,sup_var=sup_var_label,rotate=None).fit(concat((yhat,y,x),axis=1))
