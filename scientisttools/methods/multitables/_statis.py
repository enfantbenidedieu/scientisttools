# -*- coding: utf-8 -*-
from numpy import array, repeat, where, ones, ndarray, linalg, reshape, sqrt,sum,real,tile, nan, inf, insert,diff, c_, cumsum, unique
...
class STATIS(BaseEstimator,TransformerMixin):
    ...
    STATIS is a method for analysing K-tables for k groups.
    ...
    num_group_sup : list, tuple, default = None
    ...
    ind_ : ind
        ...
        coord_partiel : DataFrame of shape (n_groups*n_rows, ncp)
            The partial coordinates of the individuals.
    ...
    ind_sup_ : ind_sup, optional
        ...
        coord_partiel : DataFrame of shape (n_groups*n_rows_plus, ncp)
            The partial coordinates for the supplementary individuals.
    ...
    def fit(self,X,y=None):
        ...
        if self.num_group_sup is not None:
            ...
            # recode supplementary frequencies
            if num_group_freq is not None:
                ...
                if len(name_group_sup_freq) > 0:
                    ...
                    if len(name_group_freq) == 0 :
                        ...
                        for g, cols in group_sup_freq_dict.items():
                            ...
                            row_w_g = row_m_g/sum(row_m_g)
                            #update weights
                            Xcod_group_sup[cols] = (((P_col_sup[cols]/col_m[cols]).T - row_w_g)/row_m).T.replace([nan,inf,-inf], 1e-15)
                        ...
                        for g, cols in group_sup_freq_dict.items():
                            ...
                            row_w_g = row_m_g/sum(row_m_g)
                            #update
                            Xcod_group_sup[cols] = (((P_col_sup_g/col_m_g).T - row_w_g)/row_m).T.replace([nan,inf,-inf], 1e-15)
        ...
        A = {g : ((model[g].call_.Z * model[g].call_.col_w).dot(model[g].call_.Z.T)) * model[g].call_.row_w for g in list(group_dict.keys())}
        ...
        group_ = {"traceRV":traceRV,"RV":RV,"eig":rv_eig,"evd":namedtuple("evdResult",["V","d"])(rv_eigvects,rv_eigvals),
        ...
        # columns weights in STATIS
        col_w = concat((model[g].call_.col_w*beta[g]*alpha[g] for g in list(group_dict.keys())),axis=0)
        ...
        call_ = {"Xtot":Xtot,"X":X,"Zcod":Zcod,"Z":Z,"total":total,"row_w":row_w,"var_w":var_w,"col_w":col_w,"alpha":alpha,"beta":beta,
                 "center":center,"scale":scale,"z_center":z_center,"ncp":ncp,"group":group,"type_group":type_group,"name_group":name_group,
                 "num_group_sup":num_group_sup,"var_group":var_group,"col_group":cols_group,"ind_sup":ind_sup_label}
        #convert to namedtuple
        ...
        for g, cols in columns_dict.items():
            ...
            data[cols] = Z[cols]
            coord_partiel = ((data - z_center)*col_w).dot(self.svd_.V[:,:ncp])/alpha[g]
            # set index and column
            coord_partiel.columns = self.eig_.index[:ncp]
            coord_partiel.index = [f"{x}.{g}" for x in Z.index]
            # concatenate
        ...
        for g in name_group:
            coord = ((model[g].ind_.coord/model[g].svd_.vs[:mncp[g]]).T * row_w).dot(self.svd_.U[:,:ncp])
            coord.columns, coord.index = self.eig_.index[:ncp], [f"{x}.{g}" for x in coord.index]
            for i in range(model[g].call_.ncp):
                if coord.iloc[i,i] < 0:
                    coord.iloc[i,:] = -1*coord.iloc[i,:]
            # concatenate
            partial_axes_coord = concat_empty(partial_axes_coord,coord,axis=0)
        # cos2 for the partial axes
        partial_axes_sqcos = partial_axes_coord ** 2
        # absolute contributions for the partial axes
        ...
        for g in name_group:
            ...
            if g in list(group_dict.keys()):
                ctr = beta[g]*alpha[g]*((partial_axes_sqcos.loc[index,:]).T * model[g].eig_.iloc[:mncp[g],0].to_numpy()).T
            partial_axes_ctr.loc[index,:] = ctr
        # relative contributions for the partial axes
        ...
        #correlation between
        cor_partial_axes = wcorr(all_coord,w=row_w)
        ...
        if "quali" in type_group_var.to_numpy():
            ...
            #convert to ordered dictionary
            quali_var_ = {"coord":quali_var_coord,"coord_partiel":quali_var_coord_partiel,"contrib":quali_var_ctr}
            #convert to namedtuple
        ...
        if self.ind_sup is not None:
            ...
            for g, cols in group_dict.items():
                if self.type_group[name_group.index(g)] in ("c","s"):
                    ...
                    Zcod_ind_sup[dummies_cols] = (dummies_ind_sup - center[dummies_cols])/scale[dummies_cols]
            ...
            for g, cols in columns_dict.items():
                ...
                coord_partiel.columns, coord_partiel.index = self.eig_.index[:ncp], [f"{x}.{g}" for x in ind_sup_label]
                # concatenate
