...
def lrtest(rho,n_samples,n_xcols,n_ycols):
    """
    Likelihood ratio test
    ...
    fvalue, pvalue = empty((s,),dtype=float), empty((s,),dtype=float)
    dof1, dof2 = empty((s,),dtype=float), empty((s,),dtype=float)
    
    de = n_samples - 1.5 - 0.5*(n_xcols + n_ycols)
    for i in range(s):
        dof1[i] = (n_xcols - i)*(n_ycols - i)
        if (n_xcols - i) == 1 or (n_ycols - i) == 1:
            nu = 1 
        else :
            nu = sqrt((dof1[i]**2-4)/((n_xcols - i)**2+(n_ycols - i)**2-5))
        dof2[i] = de*nu - dof1[i]/2 + 1
        fvalue[i] = (dof2[i]/dof1[i])*((1-(value[i]**(1/nu)))/(value[i]**(1/nu)))
        pvalue[i] = stats.f.sf(fvalue[i],dof1[i],dof2[i])
    #convert to DataFrame
    statistic = DataFrame(c_[value,fvalue,dof1,dof2,pvalue],
                          columns=["Likelihood Ratio","Approximate F value","Num DF","Den DF","Pr>F"],
                          index=[f"Can{x+1}" for x in range(s)])
...
def hotelling_test(rho,n_samples,n_xcols,n_ycols):
    """
    ...
    for i in range(s):
        dof1[i] = (n_xcols - i)*(n_ycols - i)
        dof2[i] = s*(n_samples  - 2 - n_xcols - n_ycols + 2*i) + 2
        fvalue[i] = value[i]/s/dof1[i]*dof2[i]
        pvalue[i] = stats.f.sf(fvalue[i],dof1[i],dof2[i])
    #convert to DataFrame
    statistic = DataFrame(c_[value,fvalue,dof1,dof2,pvalue],
                          columns=["Value","Approximate F value","Num DF","Den DF","Pr>F"],
                          index=[f"Can{x+1}" for x in range(s)])
...
def pillai_test(rho,n_samples,n_xcols,n_ycols):
    """
    ...
    for i in range(s):
        dof1[i] = (n_xcols - i)*(n_ycols - i)
        dof2[i] = s*(n_samples - 1 + s - n_xcols - n_ycols + 2*i) 
...
def roy_test(rho,n_samples,n_xcols,n_ycols):
    """
    ...
    #convert to DataFrame
    statistic = DataFrame([[value,fvalue,dof1,dof2,pvalue]],
                          columns=["Value","Approximate F value","Num DF","Den DF","Pr>F"])
    statistic[["Num DF","Den DF"]] = statistic[["Num DF","Den DF"]].astype(int)
