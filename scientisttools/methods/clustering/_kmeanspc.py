...
class KMeansPC(BaseEstimator,TransformerMixin):
    ...
    ----------
    ncl : int.  default = 3
        If a (positive) integer, the tree is cut with ncl clusters. If None, ncl is set to 3.
    ...
    **kwargs : key words parameters
    ...
    ----------
    axes_ : axes
        An object containing the description of the clusters by the principal components. See :class:`~scientisttools.catdes`.
    ...
    call_ : call
        ...
        X : DataFrame of shape (n_samples, ncp)
            Coordinates of the samples (individuals/rows).

        data_clust : DataFrame of shape (n_samples, ncp + 1)
            The principal components with a supplementary column called cluster containing the partition.
    ...
    def fit(self,obj,y=None,sample_weight=None):
        ...
        call_ = {"obj":obj,"X":X,"D":D,"data_clust":concat((X,cluster),axis=1),"ncl":ncl,"w":w,"proba":proba,"km":km}
        ...
        # convert to ordered dictionary
        cluster_ = {"coord":cluster_coord,"inertia":km.inertia_}
        # convert to namedtuple
