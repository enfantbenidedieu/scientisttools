...
class PCAiv(BaseEstimator,TransformerMixin):
    """
    Principal Component Analysis with Instrumental Variables (PCAiv)
    ----------------------------------------------------------------
    ...
    2. Principal Component Analysis of the set of predicted values from the regression in 1.

    Usage
    -----
    ```python
    >>> PCAIV(iv = None, n_components = 5, ind_weights = None, var_weights, ind_sup = None, quanti_sup = None)
    ```
    ...
    `call_`: a namedtuple with some informations:
        ...
        * `X`: a pandas DataFrame with instrumental variables
        * `Y`: a pandas DataFrame with dependent variables
    ...
    `model_`: string specifying the model fitted = 'pcaiv'
    ...
    * Lebreton, J. D., Sabatier, R., Banco G. and Bacou A. M. (1991) Principal component and correspondence analyses with respect to instrumental variables : an overview of their role in studies of structure-activity and species- environment relationships. In J. Devillers and W. Karcher, editors. Applied Multivariate Analysis in SAR and Environmental Studies, Kluwer Academic Publishers, 85--114.
    ...
    See Also
    --------
    `get_pcaiv_ind`, `get_pcaiv_var`, `get_pcaiv`, `summaryPCAiv`, `fviz_pcaiv_ind`, `fviz_pcaiv_var`, `fviz_pcaiv_biplot`
    ...
    def fit(self,X:DataFrame,y=None):
        ...
        x, y = X.loc[:,iv_label], X.drop(columns=iv_label)

        if not all(is_numeric_dtype(y[k]) for k in y.columns): #check if all active variables are numerics
            raise TypeError("Dependent variables should all be numerics")
        ...
        ind_weights, var_weights =  Series(ind_weights,index=y.index,name="weight"), Series(ones(n_cols),index=y.columns,name="weight")

        #---------------------------------------------------------------------------------------------------------------------------------------------------------------------
        #standardization: Z = (X - mu)/sigma
        #---------------------------------------------------------------------------------------------------------------------------------------------------------------------
        #compute weighted average and standard deviation
        center = Series(average(y,axis=0,weights=ind_weights),index=y.columns,name = "center")
        scale = Series(array([sqrt(cov(y.iloc[:,k],rowvar=False,aweights=ind_weights,ddof=0)) for k in range(n_cols)]),index=y.columns,name = "scale")
        #standardization : Z = (X - mu)/sigma
        ys = y.sub(center,axis=1).div(scale,axis=1)
        ...
        Xs = x.copy()
        for k in Xs.columns:
            if is_numeric_dtype(Xs[k]):
                #compute weighted average and weightesstandard deviation
                k_center, k_scale = average(Xs[k],weights=ind_weights), sqrt(cov(Xs[k],aweights=ind_weights,ddof=0))
                #standardization : Z = (X - mu)/sigma
                Xs[k] = Xs[k].sub(k_center).div(k_scale)
        ...
        yhat = concat((olsiv(k=k,y=ys,x=Xs, weights=ind_weights) for k in ys.columns),axis=1) 

        #---------------------------------------------------------------------------------------------------------------------------------------------------------------------
        #center yhat
        #---------------------------------------------------------------------------------------------------------------------------------------------------------------------
        #apply non-normed principal component analysis
        yhat_center = Series(average(yhat,axis=0,weights=ind_weights),index=yhat.columns,name="center")
        ...
        call_ = OrderedDict(Xtot=Xtot,X=X,Z=Z,Yhat=yhat,Xs=Xs,iv=iv_label,ind_weights=ind_weights,var_weights=var_weights,center=center,scale=scale,
        ...
        ys_center = Series(average(ys,axis=0,weights=ind_weights),index=ys.columns,name="center")
        ys_scale = Series(ones(n_cols),index=ys.columns,name="scale")
        #standardization: Z = (X - mu)/sigma
        zs = ys.sub(ys_center,axis=1).div(ys_scale,axis=1)
        ...
        pca = gsvd(X=ys,row_weights=ind_weights,col_weights=var_weights,n_components=zs_max_components)
        ...
        if self.quanti_sup is not None:
            ...
            #standardization: Z = (X - mu)/sigma
            ys_quanti_sup = X_quanti_sup.sub(center_sup,axis=1).div(scale_sup,axis=1)
            #ordinaly least squared with instrumental variables
            yhat_quanti_sup = concat((olsiv(k=k,y=ys_quanti_sup,x=Xs,weights=ind_weights) for k in ys_quanti_sup.columns),axis=1)
            #compute weighted average for supplementary quantitative variables
            yhat_quanti_sup_center = Series(average(yhat_quanti_sup,axis=0,weights=ind_weights),index=yhat_quanti_sup.columns,name="center")
            yhat_quanti_sup_scale = Series(ones(n_quanti_sup),index=yhat_quanti_sup.columns,name="scale")
            #standardization: Z = (X - mu)/sigma
            Z_quanti_sup = yhat_quanti_sup.sub(yhat_quanti_sup_center,axis=1).div(yhat_quanti_sup_scale,axis=1)
            #statistics for supplementary quantitative variables
