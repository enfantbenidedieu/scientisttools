...
class CA(BaseEstimator,TransformerMixin):
    ...
    call_ : call
        ...
        Xtot : DataFrame of shape (n_rows + n_rows_sup, n_columns + n_columns_sup + n_sup_var)
            Input data.
    ...
    svd_ : svd
        ...
        d : 1d numpy array of shape (rank,)
            The eigen values (= squared singular values).
    ...
    def fit(self,X,y=None):
        ...
        if ((self.ref is not None) and 
            not isinstance(self.ref,(int,str))):
        ...
        ref_label   = get_sup_label(X=X,indexes=self.ref,axis=1)
        ...
        #rows sums and total
        if self.ref is None: 
            row_s, total = X.sum(axis=1), X.sum(axis=0).sum()
        else: 
            row_s, total = t.copy(), t.sum()
        ...
        if self.symmetric:
            ...
            col_w = Series(ones(n_cols)/n_cols,index=X.columns)
            Zcod  = (X.apply(lambda x : x/sum(x) if sum(x)!=0 else col_m,axis=1) - col_m) * n_cols
        ...
        if self.group is not None:
            ...
            if self.option == "between":
                tab = bary.copy()
                row_w = Series([row_m.loc[y[y==k].index].sum() for k in uq_classe],index=uq_classe,name="weight")
            else:
                tab, row_w = Z - bary.loc[y.to_numpy(),:].to_numpy(), row_m.copy()
        ...
        #extract elements
        self.svd_, self.eig_, self.col_ = fit_.svd, fit_.eig, namedtuple("col",fit_.col.keys())(*fit_.col.values())
        # reset number of components kepted
        ncp = self.svd_.ncp
        
        #store call informations
        call_ = {"Xtot":Xtot,"X":X,"Zcod":Zcod,"Z":Z,"bary":bary,"tab":tab,"total":total,
                 "row_s":row_s,"col_s":col_s,"row_m":row_m,"col_m":col_m,"row_w":row_w,"col_w":col_w,
                 "ncp":ncp,"iv":iv_label,"group":group_label,"ref":ref_label,
        ...
        if self.row_sup is not None:
            ...
            if self.symmetric:
                ...
                if self.ref is None:
                    row_sup_m = X_row_sup.sum(axis=1)/total
                else:
                    row_sup_m = t_row_sup/total
            ...
            if self.iv is not None:
                ...
                #extract elements
                z_row_sup_quanti_var, z_row_sup_quali_var = split_z_row_sup.quanti, split_z_row_sup.quali
                nz_row_sup_quanti_var, nz_row_sup_quali_var =split_z_row_sup.k1, split_z_row_sup.k2
                ...
                #residuals for CAoiv
                if self.ortho: 
                    Z_row_sup = Zcod_row_sup - Z_row_sup.to_numpy()
                
            #within class analysis - suppress within effect
            if self.group is not None and self.option == "within":
                Z_row_sup = Z_row_sup - bary.loc[y_row_sup.to_numpy(),:].to_numpy()
        ...
        if self.col_sup is not None:
            ...
            if self.group is not None:
                ...
                if self.option == "between":
                    Z_col_sup = bary_col_sup
                else:
                    Z_col_sup = Z_col_sup - bary_col_sup.loc[y.to_numpy(),:].to_numpy()
    ...
    def fit_transform(self,X,y=None):
        ...
        X_new : DataFrame of shape (n_rows, ncp)
            Transformed values, where ``n_rows`` is the number of rows 
