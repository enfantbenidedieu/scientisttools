...
class MCA(BaseEstimator,TransformerMixin):
    ...
    Performs Multiple Correspondence Analysis (MCA) and its derivatives (speMCA, MCAiv, MCAoiv) with supplementary individuals and supplementary variables (continuous and/or categorical).
    ...
    :class:`~scientisttools.MCA` also performs between-class/within-class analysis for Multiple Correspondence Analysis and its derivatives.
    ...
    excl_sup : None, list, default = None
        The supplementary "junk" categories. It can be a list or a tuple of the names of the supplementary categories or a 
        list or a tuple of the indexes in the supplementary disjunctive table.
    ...
    def fit(self,X,y=None):
        ...
        #set columns weights
        #---------------------------------------------------------------------------------------------------------------------------------------------------------------------
        nb_moda = X.nunique().to_numpy()
        col_w = Series([x*y for x,y in zip(p_k,repeat(var_w.to_numpy(),nb_moda))],index=Zcod.columns,name="weight")
        ...
        call_ = {"Xtot": Xtot, "X": X, "dummies": dummies, "Zcod": Zcod, "Z": Z, "bary": bary, "tab": tab, 
                 "ind_w": ind_w, "row_w": row_w, "var_w": var_w, "col_w": col_w, "center": p_k, "scale": p_k, "ncp": ncp,
