...
from .functions.recodevar import recodevarpcamix
...
class PCAmix(BaseEstimator,TransformerMixin):
    ...
    Description
    -----------
    Performs Principal Components Analysis of Mixed Data (PCAMIX) with supplementary individuals and/or supplementary variables (quantitative and/or qualitative). 
    ...
    -----
    ```python 
    >>> PCAMIX(n_components = 5, ind_weights = None, quanti_weights = None, quali_weights = None, ind_sup = None, sup_var = None, parallelize = False)
    ```
    ...
    `model_`: a string indicating the model fitted = 'pcamix'
    ...
    * Rakotomalala, Ricco (2020), Pratique des méthodes factorielles avec Python. Université Lumière Lyon 2, Version 1.0
    ...
    >>> res_pca = PCAMIX(ind_sup=(41,42,43,44,45),sup_var=(10,11,12))
    >>> res_pca.fit(decathlon)
    >>> #MCA with PCAMIX function
    >>> res_mca = PCAMIX(ind_sup=(27,28,29,30,31,32),sup_var=(6,7))
    >>> res_mca.fit(canines)
    >>> #Mixed Data with PCAMIX function
    >>> res_mix = PCAMIX(ind_sup=(38,39,40,41,42,43,44),sup_var=(12,13,14,15))
    ...
    def fit(self,X:DataFrame, y=None):
        ...
        #recode variables
        rec = recodevarpcamix(X=X,weights=ind_weights)
        ...
        if n_quali > 0:
            ...
            mod_weights = array([(1/x)*y for x,y in zip(p_k,quali_weights2)])
            #convert to Series
        ...
        col_coord = mapply(fit_.col["coord"],lambda x : x*var_weights,axis=0,progressbar=False,n_workers=n_workers)

        #statistics for active quantitative variables
        if n_quanti > 0:
            quanti_var_coord, quanti_var_ctr, quanti_var_sqcos, quanti_var_infos = col_coord.iloc[:n_quanti,:], fit_.col["contrib"].iloc[:n_quanti,:], fit_.col["cos2"].iloc[:n_quanti,:], fit_.col["infos"].iloc[:n_quanti,:]
        ...
        if n_quali > 0:
            levels_coord, levels_ctr, levels_sqcos, levels_infos = col_coord.iloc[n_quanti:,:], fit_.col["contrib"].iloc[n_quanti:,:], fit_.col["cos2"].iloc[n_quanti:,:], fit_.col["infos"].iloc[n_quanti:,:]
        ...
        if all(x > 0 for x in [n_quanti,n_quali]):
            #qualitative variables contributions in PCAMIX
            ...
            #qualitative variables square cosinus in PCAMIX
        ...
        if self.sup_var is not None:
            ...
            rec_sup_var = recodevarpcamix(X=X_sup_var,weights=ind_weights)
            #extract elements
            ...
            if n_quali_sup > 0:
                p_k_sup = mapply(dummies_sup,lambda x : x*ind_weights,axis=0,progressbar=False,n_workers=n_workers).sum(axis=0) #proportion levels
                n_k_sup = p_k_sup*n_rows #count levels
                #coordinates for the supplementary levels
                levels_sup_coord = mapply(col_sup_coord.iloc[n_quanti_sup:,:],lambda x : x/p_k_sup, axis=0,progressbar=False,n_workers=n_workers)
                ...
                #dist2 and cos2 for the supplementary levels
                levels_sup_sqdisto, levels_sup_sqcos = col_sup_sqdisto.iloc[n_quanti_sup:], col_sup_sqcos.iloc[n_quanti_sup:,:]
                #convert to ordered dictionary
    ...
    def transform(self,X:DataFrame,y=None) -> DataFrame:
        ...
        if len(intersect_col) != self.call_.X.shape[1]:
            raise ValueError("The names of the variables is not the same as the ones in the active variables of the PCAMIX result")
...
def predictPCAMIX(self,X:DataFrame) -> NamedTuple:
    """
    ...
    Description
    -----------
    Performs the coordinates, squared cosinus and squared distance to origin of new individuals with Principal Component Analysis of Mixed Data (PCAMIX)

    Usage
    -----
    ```python
    >>> predictPCAMIX(self,X)
    ```
    ...
    >>> from scientisttools import PCAMIX, predictPCAMIX
    ...
    >>> predict = predictPCAMIX(res_pcamix,X=X_ind_sup)
    ...
    ```
    """
    if self.model_ != "pcamix": #check if self is an object of class PCAMIX
    ...
    intersect_col = list(set(X.columns.tolist()) & set(self.call_.X.columns.tolist())) #find intersect
    if len(intersect_col) != self.call_.X.shape[1]:
        raise ValueError("The names of the variables are not the same as the ones in the active variables of the PCAMIX result")
    #reorder columns
    ...
    return namedtuple("predictPCAMIXResult",["coord","cos2","dist2"])(coord,sqcos,sqdisto)
        
def supvarPCAMIX(self,X:DataFrame) -> NamedTuple:
    """
    ...
    ```python
    >>> from scientisttools.datasets import load_autos2005
    >>> from scientisttools import PCAMIX, supvarPCAMIX
    ...
    >>> sup_var_predict = supvarPCAMIX(res_pcamix,X=X_sup_var)
    ...
    ```
    """
    if self.model_ != "pcamix": #check if self is and object of class PCAMIX
        raise TypeError("'self' must be an object of class PCAMIX")
    ...
    #recode supplementary variables
    rec = recodevarpcamix(X=X,weights=self.call_.ind_weights)
    #extract elements
    ...
    #statistics for supplementary qualitative variables/levels
    if n_quali > 0:
        p_k = mapply(dummies,lambda x : x*self.call_.ind_weights,axis=0,progressbar=False,n_workers=self.call_.n_workers).sum(axis=0) #proportion levels
        n_k = p_k*n_rows #count levels
        #coordinates for the supplementary levels
        levels_coord = mapply(col_coord.iloc[n_quanti:,:],lambda x : x/p_k, axis=0,progressbar=False,n_workers=self.call_.n_workers)
        #vtest for the supplementary levels
        ...
        #dist2 and cos2 for the supplementary levels
        levels_sqdisto, levels_sqcos = col_sqdisto.iloc[n_quanti:], col_sqcos.iloc[n_quanti:,:]
        #convert to ordered dictionary
    ...
    #convert to namedtuple
    return namedtuple("supvarPCAMIXResult",["quanti","quali"])(quanti_sup,quali_sup)