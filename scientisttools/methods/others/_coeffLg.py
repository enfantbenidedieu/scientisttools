...
def coeffLg(X, 
    ...
    Performs Lg coefficients between groups. Groups of variables can be continuous, categorical or mixed. Missing values on continuous variables are replaced by the column mean. Missing values on categorical variables are replaced by the most frequent categories in columns.
    ...
    Lg : Dataframe of shape (n_groups, n_groups)
        The \emph{Lg} coefficients matrix.

    Examples
    --------
    >>> from scientisttools.datasets import wine, poison, friday87, poison
    >>> from scientisttools import coeffLg
    >>> Lg = coeffLg(X=wine.data,group=wine.group,type_group=("n","s","s","s","s","s"),name_group=wine.name)
    >>> Lg = coeffLg(X=poison.data,group=poison.group,type_group=("s","n","n","n"),name_group=poison.name)
    >>> Lg = coeffLg(X=friday87.data,group=friday87.group,type_group=("f","f","f","f","f","f","f","f","f","f"),name_group=friday87.name)
    >>> Lg = coeffLg(X=poison.data.iloc[:20,:],group=poison.group,type_group=("s","m","n","s"),name_group=poison.name)
    """
    #---------------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    Lg = DataFrame(index=name_group,columns=name_group).astype(float)
