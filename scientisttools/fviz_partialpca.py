...
def fviz_partialpca3d_ind(self,
                            axis=[0,1,2],
                            x_lim=None,
                            y_lim=None,
                            x_label = None,
    ...
    """
    Visualize Partial Principal Component Analysis (PartialPCA) - 3D graph of individuals
    -------------------------------------------------------------------------------------

    Usage
    -----
    ```python
    >>> fviz_partialpca3d_ind(self,
                                axis=[0,1,2],
                                x_lim = None,
                                y_lim = None,
                                x_label = None,
    ...
    >>> from scientisttools import load_cars2006
    >>> D = load_cars2006(which="actif")
    >>> from scientisttools import PartialPCA, fviz_partialpca3d_ind
    >>> res_partialpca = PartialPCA(n_components=None,standardize=True,partial=["CYL"],parallelize=False)
    >>> res_partialpca.fit(D)
    ...
    if habillage is None :  
        if (isinstance(color,str) and color in [*["cos2","contrib"],*coord.columns]):
            ...
            if "text" in geom:
                if repel :
                    p = p + text3d_label(text_type,mapping=pn.aes(color=color),size=text_size,va=va,ha=ha,adjust_text={'arrowprops': {'arrowstyle': '-','lw':1.0}})
                else:
            ...
            if "text" in geom:
                if repel :
                    p = p + text3d_label(text_type,mapping=pn.aes(color="num_var"),size=text_size,va=va,ha=ha,adjust_text={'arrowprops': {'arrowstyle': '-','lw':1.0}})
                else:
                    p = p + text3d_label(text_type,mapping=pn.aes(color=c),size=text_size,va=va,ha=ha)
        elif hasattr(color, "labels_"):
            if "point" in geom:
                p = (p + pn3d.geom_point_3d(pn.aes(color="cluster",linetype = "cluster"),size=point_size)+pn.guides(color=pn.guide_legend(title=legend_title)))
            if "text" in geom:
                if repel :
                    p = p + text3d_label(text_type,mapping=pn.aes(color="cluster"),size=text_size,va=va,ha=ha,adjust_text={'arrowprops': {'arrowstyle': '-','lw':1.0}})
                else:
                    p = p + text3d_label(text_type,mapping=pn.aes(color=c),size=text_size,va=va,ha=ha)
        else:
            if "point" in geom:
            ...
            if "text" in geom:
                if repel :
                    p = p + text3d_label(text_type,color=color,size=text_size,va=va,ha=ha,adjust_text={'arrowprops': {'arrowstyle': '-','color': color,'lw':1.0}})
                else:
    ...
    # Set title
    if title is None:
        title = "Individuals 3D Factor Map - PartialPCA"
...
def fviz_partialpca(self,choice="biplot",**kwargs)->pn:
    ...
    if choice == "ind":
        ...
        return fviz_partialpca3d_ind(self,**kwargs)