...
def get_pcamix_ind(self) -> NamedTuple:
    """
    ...
    `coord`: coordinates of individuals.
...
def get_pcamix_var(self, element = "var") -> NamedTuple:
    """
    ...
    Description
    -----------
    Extract all the results (coordinates, squared cosinus and relative contributions) for quantitative and qualitative variables from Principal Component Analysis of Mixed Data (PCAMIX) outputs.

    Usage
    -----
    ```python
    >>> get_pcamix_var(self,element="var")
    >>> get_pcamix_var(self,element="quanti_var")
    >>> get_pcamix_var(self,element="quali_var")
    ```
    ...
    `element`: the element to subset from the output. Possible values are:
        ...
        * "quali_var" for active qualitative variables (categories)
    ...
    if element == "quanti_var":
        if not hasattr(self,"quanti_var_"):
            raise ValueError("No quantitative variables")
        return self.quanti_var_
    
    if element == "quali_var":
        if not hasattr(self,"quali_var_"):
            raise ValueError("No qualitative variables")
...
def get_pcamix(self,element = "ind")-> NamedTuple:
    """
    ...
    Description
    -----------
    Extract all the results (coordinates, squared cosine and relative contributions) for the individuals and variables from Principal Component Analysis of Mixed Data (PCAMIX) outputs.

    Usage
    -----
    ```python
    >>> get_pcamix(self, elemet = "ind")
    >>> get_pcamix(self, elemet = "var")
    >>> get_pcamix(self, elemet = "quanti_var")
    >>> get_pcamix(self, elemet = "quali_var")
    ```
    ...
    `elemeny`: the element to subset from the output. Possibles values are :
        * "ind" for individuals
        * "var" for active variables
    ...
    Return
    ------
    a namedtuple of pandas DataFrames containing the results for the active individuals and variables, including :
    ...
    if element == "ind":
        ...
        return get_pcamix_var(self,element=element)
    else:
        Exception("'element' should be one of 'ind', 'quanti_var', 'quali_var', 'var'")
