...
def get_mfa_ind(obj):
    ...
    result : ind
        ...
        within_inertia : DataFrame of shape of shape (n_samples, ncp)
            Within inertia of the individuals.
        within_partial_inertia: DataFrame of shape (n_samples*n_groups, ncp)
            Partial within inertia of the inertia.
            
    See Also
    --------
    get_mfa : Extract the results for individuals/variables/groups/frequencies/partial axes - MFA
    get_mfa_freq : Extract the results for frequencies - MFA
    get_mfa_group : Extract the results for groups - MFA
    get_mfa_partial_axes : Extract the results for partial axes - MFA
    get_mfa_quanti_var : Extract the results for quantitative variables - MFA
    get_mfa_quali_var : Extract the results for variables categories - MFA
...
def get_mfa_quanti_var(obj):
    """
    Extract the results for quantitatie variables - MFA
    
    Extract all the results (coordinates, squared cosinus and relative contributions) for the active continuous variables from Multiple Factor Analysis (MFA) outputs.
    ...
    get_mfa_freq : Extract the results for frequencies - MFA
    get_mfa_group : Extract the results for groups - MFA
    get_mfa_ind : Extract the results for individuals - MFA
    get_mfa_partial_axes : Extract the results for partial axes - MFA
    get_mfa_quali_var : Extract the results for qualitative variables/levels - MFA
...
def get_mfa_quali_var(obj):
    """
    Extract the results for variables categories - MFA
   
    Extract all the results (coordinates, squared cosinus and relative contributions) for the active variables categories from Multiple Factor Analysis (MFA) outputs.
    ...
    result : levels
        ...
        within_inertia : DataFrame of shape of shape (n_levels, ncp)
            Within inertia of the levels.
        within_partial_inertia: DataFrame of shape (n_levels*n_groups, ncp)
            Partial within inertia of the inertia for levels
    ...
    >>> clf = MFA(group=(2,2,5,6),group_type=("s","n","n","n"),name_group=poison.name,num_group_sup=(0,1))
    >>> mfa.fit(poison.data)
...
def get_mfa(obj, element = "group"):
    """
    Extract the results for individuals/variables/groups/frequencies/partial axes - MFA
    
    Extract all the results (coordinates, squared cosinus and relative contributions) for the active individuals/variables/groups/frequencies/partial axes from Multiple Factor Analysis (MFA) outputs.
    ...
    element : {"ind", "quanti_var", "group", "quali_var", "freq", "partial_axes"}, default = "group"
