...
class STATIS(BaseEstimator,TransformerMixin):
    ...
    Performs STATIS method with supplementary individuals and supplementary groups of variables. 
    Active groups of variables must be quantitative. Supplementary groups 
    ...
    def fit(self,X,y=None):
       ...
       ######################################## Drop supplementary groups columns #######################################
        ...
        for grp,cols in group_active_dict.items():
            ...
            ################################ variables weights
            weights = pd.Series(np.repeat(a=1/np.sqrt(np.sum(model[grp].eig_.iloc[:,0]**2)),repeats=len(cols)),index=cols)
