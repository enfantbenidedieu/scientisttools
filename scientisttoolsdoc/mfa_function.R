...
MFA2 <- function (base, group, type = rep("s",length(group)), excl = NULL, ind.sup = NULL, ncp = 5, name.group = NULL, num.group.sup = NULL, graph = TRUE, weight.col.mfa = NULL, row.w = NULL, axes=c(1,2),tab.comp=NULL){
  ...
  funcLg <- function (x, y, ponderation.x, ponderation.y, wt = rep(1/nrow(x), nrow(x)), cor = FALSE) {
    ...
    else if (!is.matrix(x)) stop("'x' must be a matrix or a data frasme")
  ...
  for (gl in c(group.actif,num.group.sup)) {
    ...
    for (gc in c(group.actif,num.group.sup)) {
      if (gc>=gl){
        if (is.null(num.group.sup)) {
          if (is.null(ind.sup)){
            Lg[gl, gc] <- Lg[gc, gl] <- funcLg(x=data[,ind.gl + (1:group.mod[gl]),drop=FALSE],y=data[,ind.gc + (1:group.mod[gc]),drop=FALSE],ponderation.x=ponderation[ind.gl + (1:group.mod[gl])],ponderation.y=ponderation[ind.gc + (1:group.mod[gc])],wt=row.w/sum(row.w))
...
# 
# res=MFA2(Biometrie2,group=c(3,3,3,3),
#         type=c("n","s","n","s"),
#         num.group.sup = c(3,4),
#         graph = FALSE)

# data(mortality)
# res<-MFA2(mortality,group=c(9,9),type=c("f","f"),
